...
def fill_mask(y_trn, m_trn):
    ...
    y_add = np.array([[1 if (m_trn[idx, idy] == 0) and (y_pos[idy] > y_neg[idy])
                     else 0 for idy in range(y_trn.shape[1])] for idx in range(y_trn.shape[0])])
...
def bin2idx(omic_bin):
    ...
    # max num of mutation in a single sample
    num_max_omic = omic_bin.sum(axis=1).max()
...
def get_ptw_ids(drug_info, tgt, repository):
    ...
    id2pw = {id: pw for id, pw in zip(
        drug_info.index, drug_info['Target pathway'])}
...
def load_dataset(input_dir="data/input", repository="gdsc", drug_id=-1, shuffle_feature=False):
    ...
    drug_info = pd.read_csv(os.path.join(
        input_dir, 'drug_info_'+repository+'.csv'), index_col=0)
    ...
    # mask of target data: 1->data available, 0->nan
    msk = tgt.notnull().astype(int).values
    # fill nan element of target with 0.
    tgt = tgt.fillna(0).astype(int).values
...
def get_accuracy(aryx, aryy, mskx, msky):
    acc_list = [aryx[i] == aryy[i]
                for i in range(len(aryx)) if (mskx[i] == 1) and (msky[i] == 1)]
...
def get_minibatch(dataset, rng, index, batch_size, batch_type="train", use_cuda=True):
    ...
    if batch_type == "train":
        batch_dataset = {
            k: [dataset[k][rng[i % size_rng]]
                for i in range(index, index+batch_size)]
            for k in dataset.keys()}
    elif batch_type == "test":
        batch_dataset = {
            k: [dataset[k][rng[i]]
                for i in range(index, min(index+batch_size, size_rng))]
            for k in dataset.keys()}
...
def evaluate(labels, msks, preds, epsilon=1e-5, mode='classification'):
    ...
    flat_labels_msk = np.array([flat_labels[idx]
                               for idx, val in enumerate(flat_msks) if val == 1])
    flat_preds_msk = np.array([flat_preds[idx]
                              for idx, val in enumerate(flat_msks) if val == 1])
