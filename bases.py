...
class CLR():
    ...
    def calc_lr(self, loss):
        self.iteration += 1
        # stopping criteria (if current loss > 4*best loss)
...
class OneCycle():
    def __init__(self, nb, max_lr, momentum_vals=(0.95, 0.85), prcnt=10, div=10):
        ...
        # percentage of cycle length, we annihilate learning rate below the lower learnig rate (default is 10)
        self.prcnt = prcnt
        self.iteration = 0
        self.lrs = []
        self.moms = []
...
class Base(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.output_dir = args.save_path
...
class DrugDecoder(nn.Module):
    ...
    def forward(self, hid_omc, drg_ids):
        ...
        # (batch_size, num_drg, hidden_dim_enc)
        E_t = E_t.repeat(hid_omc.shape[0], 1, 1)

        logit_drg = torch.matmul(
            hid_omc.view(hid_omc.shape[0],
                         hid_omc.shape[1], 1, hid_omc.shape[2]),
        ...
        drg_bias = drg_bias.repeat(
            hid_omc.shape[0], 1)  # (batch_size, num_drg)
...
class ExpEncoder(nn.Module):
    ...
    def __init__(
        ...
        if init_gene_emb:
            if self.repository == 'gdsc':
                # GDSC dataset
                gene_emb_pretrain = np.genfromtxt(
                    'data/input/exp_emb_gdsc.csv', delimiter=',')
            else:
                gene_emb_pretrain = np.genfromtxt(
                    'data/input/exp_emb_ccle.csv', delimiter=',')
    ...
    def forward(self, omc_idx, ptw_ids):
        ...
        if self.use_attention:
            ...
            # (batch_size, 1, num_omc, embedding_dim)
            E_t = torch.unsqueeze(E_t, 1)
            # (batch_size, num_drg, num_omc, embedding_dim)
            E_t = E_t.repeat(1, ptw_ids.shape[1], 1, 1)

            if self.use_cntx_attn:
                # (1, num_drg, attention_size)
                Ep_t = self.layer_emb_ptw(ptw_ids)
                # (1, num_drg, 1, attention_size)
                Ep_t = torch.unsqueeze(Ep_t, 2)
                # (batch_size, num_drg, num_omc, attention_size)
                Ep_t = Ep_t.repeat(omc_idx.shape[0], 1, omc_idx.shape[1], 1)

                # (batch_size, num_drg, num_omc, attention_size)
                E_t_1 = torch.tanh(self.layer_w_0(E_t) + Ep_t)

            else:
                # (batch_size, num_omc, attention_size)
                E_t_1 = torch.tanh(self.layer_w_0(E_t))

            # (batch_size, num_drg, num_omc, attention_head)
            A_omc = self.layer_beta(E_t_1)

            # (batch_size, num_drg, num_omc, attention_head)
            A_omc = F.softmax(A_omc, dim=2)
            # (batch_size, num_drg, num_omc, 1)
            A_omc = torch.sum(A_omc, dim=3, keepdim=True)
            ...
            self.Amtr = torch.squeeze(A_omc, 3)

            # (batch_size, num_drg, embedding_dim)
            emb_omc = torch.sum(torch.matmul(A_omc.permute(
                0, 1, 3, 2), E_t), dim=2, keepdim=False)

        else:

            # (batch_size, embedding_dim)
            emb_omc = torch.mean(E_t, dim=1, keepdim=False)

            # (batch_size, 1, embedding_dim)
            emb_omc = torch.unsqueeze(emb_omc, 1)

            # (batch_size, num_drg, embedding_dim)
            emb_omc = emb_omc.repeat(1, ptw_ids.shape[1], 1)
